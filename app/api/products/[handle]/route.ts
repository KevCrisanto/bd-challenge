...
export const GET = async (
  ...
  const product: ShopifyProductDetail = {
    ...
    images: raw.images.edges.map(({ node }: { node: typeof raw.images.edges[number]["node"] }) => node),
    options: raw.options,
    variants: raw.variants.edges.map(({ node }: { node: typeof raw.variants.edges[number]["node"] }) => ({
      id: node.id,
